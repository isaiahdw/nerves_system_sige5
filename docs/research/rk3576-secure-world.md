...
That window is left open on purpose.

The obvious way to close it is a second OTP word recording that the burn
finished - written only after the key has been read back and compared, and
required on every later read. It does not survive examination:

- **The completion word is itself one-way and can tear.** A power cut while
  programming it leaves a value that is neither blank nor the expected magic,
  and the read path then refuses a part whose key is perfect. That is a new
  brick mode of roughly the probability of the one it prevents - five words
  that can tear instead of four.
- **The window is already narrow.** The burn reads back and compares
  immediately after writing, so the only uncaught case is a power cut in the
  microseconds between the last word landing and the compare running, on first
  boot only.
- **It cannot be introduced to a fleet that already exists.** A part fused
  before the scheme and a part torn mid-burn both read the word as blank.
  Requiring it refuses every older part; accepting blank restores the exact
  hole it was added for. There is no setting that is both safe and compatible.

The last point is decisive on any fleet that is not covered from unit one, and
the first makes it a poor trade even on one that is.

Two things follow from that, and neither is closed:

- The RK3576 secure OTP's own integrity behaviour - ECC, parity, lock bits -
  is not established. Any scheme of this shape rests on assumptions about how
  a torn word reads, so measure that before building on it.
- Refusing a HUK costs secure storage and only secure storage. `read_huk`
  returns an error, `tee_fs_init_key_manager` logs "Secure storage will not be
  available", and OP-TEE carries on and hands off to the normal world.
  `do_init_calls` logs a failed initcall and does **not** panic - worth stating
  plainly, because it looks fatal in a boot log and is not.
...
`0x84`, `0x88` and `0x8c` remain blank and spare, which is where a retry slot
could go. No retry is implemented.
